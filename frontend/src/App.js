...
// Modal for purchasing Diamonds (virtual currency)
const DiamondsModal = ({ isOpen, onClose, packages = [], onPurchase }) => {
  if (!isOpen) return null;
  return (
    <div className="video-player-overlay" onClick={onClose} style={{ zIndex: 1000 }}>
      <div className="video-player-container" onClick={(e) => e.stopPropagation()}>
        <button className="video-player-close" onClick={onClose}>
          <X size={24} />
        </button>
        <div className="diamond-modal-content">
          <h2 style={{ marginBottom: "12px" }}>Buy Diamonds</h2>
          <div className="diamond-packages">
            {packages && packages.length > 0 ? (
              packages.map((pkg) => (
                <div key={pkg.id} className="diamond-package-card">
                  <h3>{pkg.diamonds + (pkg.bonus || 0)} Diamonds</h3>
                  {pkg.bonus && pkg.bonus > 0 && (
                    <p className="diamond-bonus">+{pkg.bonus} bonus</p>
                  )}
                  <p className="diamond-price">${pkg.price}</p>
                  <button
                    className="plan-btn primary"
                    onClick={() => onPurchase && onPurchase(pkg.id)}
                  >
                    Buy Now
                  </button>
                </div>
              ...
              <p>No diamond packages available.</p>
            )}
          </div>
        </div>
        <style>{`
          ...
          .diamond-package-card h3 {
            margin: 0 0 4px;
            font-size: 18px;
            font-weight: 600;
          }
          .diamond-package-card .diamond-bonus {
            margin: 0;
            font-size: 12px;
            color: #ecc94b;
          }
          .diamond-package-card .diamond-price {
            margin: 8px 0;
            font-size: 16px;
            font-weight: 500;
          }
  ...
  return (
    <header className={`netflix-header ${isScrolled ? "scrolled" : ""}`}>
      <div className="header-left">
        <div className="logo">
          {/* Use local logo image instead of remote placeholder */}
          <img
            src="/gizzle-logo.jpeg"
            alt="Gizzle TV"
            className="logo-img"
          />
      ...
      <div className="header-right">
        {/* Diamonds balance button */}
        {isAuthenticated && (
          <button
            className={`nav-item diamonds-btn`}
            onClick={() => onOpenDiamondsModal && onOpenDiamondsModal()}
            title="Diamonds Balance"
          >
            <Star size={16} />
            <span className="diamonds-count">{diamondsBalance ?? 0}</span>
          </button>
        )}
...
const HeroBanner = ({ onPlayClick }) => {
  ...
  return (
    <div className="hero-banner">
      {/*
        The hero background uses a CSS background-image (Option B).  The
        ``hero-background`` element is styled in App.css with a
        ``background-image: url('/gizzle-logo.jpeg')``, covering the full
        hero area.  The overlays and gradient are layered on top to
        darken the edges and maintain text readability.
      */}
      <div className="hero-background">
        {/* There is no <img> here because the background image is set via CSS. */}
...
function AppContent() {
  ...
  // Diamonds state: balance, packages and modal visibility
  const [diamondsBalance, setDiamondsBalance] = useState(0);
  ...
  // Fetch diamond packages and user balance when authenticated
  useEffect(() => {
    if (isAuthenticated) {
      // Fetch available packages
      axios
        .get(`${API}/diamonds/packages`)
        .then((res) => setDiamondPackages(res.data))
        .catch((err) => console.error('Failed to load diamond packages', err));
      // Fetch balance if user_id is available
      if (user && user.user_id) {
        axios
          .get(`${API}/diamonds/balance`, { params: { user_id: user.user_id } })
          .then((res) => setDiamondsBalance(res.data.balance))
          .catch((err) => console.error('Failed to fetch diamonds balance', err));
      }
    }
  }, [isAuthenticated]);

  // Handler to initiate diamond purchase
  const handlePurchaseDiamonds = async (packageId) => {
    ...
    try {
      const response = await axios.post(`${API}/diamonds/checkout`, null, {
        params: { package_id: packageId, user_id: user.user_id },
      });
      ...
      console.error('Failed to create diamonds checkout', err);
    }
  };
  ...
  return (
    <div className="netflix-app">
      <NetflixHeader
        ...
        onOpenDiamondsModal={() => setShowDiamondsModal(true)}
      />
      ...
      {/* Diamonds purchase modal */}
      <DiamondsModal
        isOpen={showDiamondsModal}
        onClose={() => setShowDiamondsModal(false)}
